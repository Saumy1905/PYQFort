...
function initializePDFViewer() {
  ...
  if (iframe) {
    ...
    if (isMobile) {
      // Try to force inline viewing on mobile
      ...
      setTimeout(() => {
        try {
          // If iframe content is not accessible, try object/embed fallback
          if (!iframe.contentDocument && !iframe.contentWindow) {
            console.log('Iframe not accessible, trying object fallback');
          ...
          console.log('Iframe error, trying object fallback:', e);
          iframe.style.display = 'none';
          if (pdfObject) {
            pdfObject.style.display = 'block';
          }
